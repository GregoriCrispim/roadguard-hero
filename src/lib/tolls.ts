...
function distanceToPolylineMeters(point: LatLng, polyline: [number, number][]): number {
  ...
  for (const [lat, lng] of polyline) {
    min = Math.min(min, haversineMeters(point, { lat, lng }));
  }
...
function estimateTollsByDistance(distanceMeters: number): { tolls: TollOnRoute[]; totalCents: number } {
  const km = distanceMeters / 1000;
  if (km < 20) return { tolls: [], totalCents: 0 };
...
export function calculateTollsAlongRoute(
  ...
  if (found.length > 0) {
    ...
    return { tolls: found, totalCents, hasEstimate: false };
  }

  const estimated = estimateTollsByDistance(distanceMeters);
...
/** Posições para marcadores no mapa */
export function getTollPlazaPosition(id: string): { lat: number; lng: number } | null {
  const plaza = TOLL_PLAZAS.find((p) => p.id === id);
  return plaza ? { lat: plaza.lat, lng: plaza.lng } : null;
}
